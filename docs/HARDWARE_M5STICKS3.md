...
经实测（USB-CDC `gpioscan` 命令扫描所有用户可访问 GPIO，配合人工按压）：

| 物理位置 | 功能 | GPIO | 说明 |
|---|---|---|---|
| **左侧小按键** | PMIC 侧边按键 | 走 PMIC (M5PM1, I2C 0x6E) 处理，不直接连 ESP32 GPIO | 单按=ON、双击=OFF、长按≥3s=BOOT 下载模式 |
| **中间大长条** | 唯一用户按键 | **G11** (M5.BtnA, RTC IO) | 业务交互主按键 |
| 右侧中等"按键" | **非真按键** | — | 按下无任何电气反应；很可能是 IR 接收窗或装饰孔，机械有声但电气空接 |

> ⚠️ M5StickS3 实测**只有 1 个用户可编程按键**（中间大长条 = G11）。左侧小按键被 PMIC 占用做电源/BOOT，业务层不能用；右侧空接。
> ⚠️ G12 在 M5Unified 源码里被 polling，但 M5StickS3 没接物理按键，因此 `M5.BtnB` 永远不会触发。
> ⚠️ V1 业务交互全靠 **G11 单按 + 中等长按 (1-2s) + IMU 摇一摇**。双击/超长按不能用（PMIC 拦截会关机或进 boot 模式）。
